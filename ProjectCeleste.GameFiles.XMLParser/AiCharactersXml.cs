...
namespace ProjectCeleste.GameFiles.XMLParser
{
    ...
    public class AiCharacterXml
    {
        ...
        public AiCharacterXmlTechs ActiveTechs { get; set; } = new AiCharacterXmlTechs();

        [Required]
        [JsonProperty(PropertyName = "protounits", Required = Required.AllowNull)]
        [XmlElement(ElementName = "protounits")]
        public AiCharacterXmlProtoUnits ProtoUnits { get; set; } = new AiCharacterXmlProtoUnits();
        ...
        public AiCharacterXmlAvailableAdvisors AvailableAdvisors { get; set; } = new AiCharacterXmlAvailableAdvisors();

        [Required]
        [JsonProperty(PropertyName = "assignedadvisors", Required = Required.AllowNull)]
        [XmlElement(ElementName = "assignedadvisors")]
        public AiCharacterXmlAssignedAdvisors AssignedAdvisors { get; set; } = new AiCharacterXmlAssignedAdvisors();
    ...
    public class AiCharactersXml
    {
        ...
        {
            AiCharacter = new Dictionary<string, AiCharacterXml>(StringComparer.OrdinalIgnoreCase);
        }

        [JsonIgnore]
        [XmlIgnore]
        public IDictionary<string, AiCharacterXml> AiCharacter { get; }

        /// <summary>
        ///     Use AiCharacter Dictionary Instead! Only only used for xml parsing.
        /// </summary>
        [Required]
        [JsonProperty(PropertyName = "characters", Required = Required.Always)]
        [XmlElement(ElementName = "characters")]
        public AiCharacterXml[] AiCharacterArrayDoNotUse
        {
            get => AiCharacter.Values.ToArray();
            set
            {
                AiCharacter.Clear();
                if (value == null)
                ...
                foreach (var item in value)
                    try
                    {
                        AiCharacter.Add(item.FileName, item);
                    }
                    catch (Exception e)
                    {
