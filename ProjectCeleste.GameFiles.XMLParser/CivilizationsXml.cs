...
namespace ProjectCeleste.GameFiles.XMLParser
{
    ...
    public class CivilizationsXml
    {
        public CivilizationsXml()
        {
            Civilization = new Dictionary<CivilizationEnum, CivilizationXml>();
        }

        public CivilizationsXml(
            [JsonProperty(PropertyName = "Civilization", Required = Required.Always)]
            IDictionary<CivilizationEnum, CivilizationXml> civilizations)
        {
            Civilization = new Dictionary<CivilizationEnum, CivilizationXml>(civilizations);
        }

        [JsonProperty(PropertyName = "Civilization", Required = Required.Always)]
        [XmlIgnore]
        public Dictionary<CivilizationEnum, CivilizationXml> Civilization { get; }

        /// <summary>
        ///     Use Civilization Dictionary Instead! Only only used for xml parsing.
        /// </summary>
        [JsonIgnore]
        [XmlElement(ElementName = "Civilization")]
        public CivilizationXml[] RegionArrayDoNotUse
        {
            get => Civilization.Values.ToArray();
            set
            {
                Civilization.Clear();
                if (value == null) return;
                foreach (var item in value)
                    Civilization.Add(item.Civid, item);
            }
        }
